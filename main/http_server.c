...
esp_err_t http_server_OTA_update_handler(httpd_req_t *req)
{
	...
	do
	{
		...
		if (!is_req_body_started)
		{
			...
			{
				printf("http_server_OTA_update_handler: Writing to partition subtype %d at offset 0x%08x\r\n",update_partition->subtype,(unsigned int)update_partition->address);
	...
	if (esp_ota_end(ota_handle) == ESP_OK)
	{
		...
		if (esp_ota_set_boot_partition(update_partition) == ESP_OK)
		{
			...
			ESP_LOGI(TAG, "http_server_OTA_update_handler: Next boot partition subtype %d at offset 0x%08x",boot_partition->subtype,(unsigned int)boot_partition->address);
